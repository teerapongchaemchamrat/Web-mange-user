...
export default function RepairCom () {
  ...
  const getCaseBrand = localStorage.getItem('CaseBrand');
  const getCaseModel = localStorage.getItem('CaseModel');
  const getCaseService = localStorage.getItem('CaseService');
  const getMoniterBrand = localStorage.getItem('MoniterBrand');
  const getMoniterService = localStorage.getItem('MoniterService');
  const getMoniterSerial = localStorage.getItem('MoniterSerial');
  const getKeyBrand = localStorage.getItem('KeyBrand');
  const getKeySerial = localStorage.getItem('KeySerial');
  const getMouseBrand = localStorage.getItem('MouseBrand');
  const getMouseSerial = localStorage.getItem('MouseSerial');
  const getOS = localStorage.getItem('OS');
  const getOfficeVersion = localStorage.getItem('OfficeVersion');
  const getOfficeKey = localStorage.getItem('OfficeKey');
  const getOfficeAcc = localStorage.getItem('OfficeAcc');
  const getOfficePass = localStorage.getItem('OfficePass');
  const getLocation = localStorage.getItem('Location');
  
  useEffect(() => {
    const getComDataFromLocalStorage = () => {
        setEmployeeID(getUserID);
        setCase_brand(getCaseBrand);
        setCase_model(getCaseModel);
        setCase_service(getCaseService);
        setMoniter_brand(getMoniterBrand);
        setMoniter_serial(getMoniterSerial);
        setMoniter_service(getMoniterService);
        setKey_brand(getKeyBrand);
        setKey_serial(getKeySerial);
        setMouse_brand(getMouseBrand);
        setMouse_serial(getMouseSerial);
        setOs(getOS);
        setOffice_version(getOfficeVersion);
        setOffice_key(getOfficeKey);
        setOffice_acc(getOfficeAcc);
        setOffice_pass(getOfficePass);
        setLocation(getLocation);
    };

    getComDataFromLocalStorage();
  }, [
    getUserID, getCaseBrand, getCaseModel, getCaseService, getMoniterBrand, getMoniterService, getMoniterSerial,
    getKeyBrand, getKeySerial, getMouseBrand, getMouseSerial, getOS, getOfficeVersion, getOfficeKey, getOfficeAcc,getOfficePass,getLocation
  ]);
      ...
      content: (
        <div style={{ marginTop: '-6rem', marginBottom: '-6rem', marginLeft: '1rem'}}>
          <div style={{display: 'flex', marginLeft: '30%' }}>
            ...
            <div style={{width: '250px', marginLeft: '4.8%'}} >
            <Input id="standard-basic" placeholder='User ID' 
                       value={employeeID}
  ...
  console.log('userData:', JSON.stringify(comData, null, 2));

    try{
        if(!employeeID){
            setCurrent(0);
            message.error('กรุณากรอกรหัสผู้ใช้งาน');

        }else {
            setloading(true);
            await axios.put(`http://192.168.10.114:88/bcc/com/update/${getCaseService}`, comData);
