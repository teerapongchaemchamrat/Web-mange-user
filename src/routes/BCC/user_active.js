...
import Adduser from '../BCC/user_add';
import Edituser from '../BCC/user_edit';
...
export default function ManageUser () {
  ...
  const getData = async () => {
    try{
        setloading(true);
        const getuser = await axios.get('http://192.168.10.114:88/bcc/user/active');
  ...
  const showModalRemove = async (record) => {
    ...
    try{
      setloading(true);
      await axios.put(`http://192.168.10.114:88/bcc/user/remove/${record.user_id}`, Data);
    ...
    const content = (
      <Document>
        <Page size="A4" style={styles.page}>
          ...
          <Image src="/image/BCC_userform.jpg" style={styles.backgroundImage} />
        </Page>
      </Document>
    );
