...
export default function Adduser () {
  ...
  const getListID = async() => {
    try {
      const getlist = await axios.get('http://192.168.10.114:88/bcc/user/get');
      const response = getlist.data;

      if (response.length > 0) {
          const lastUserID = response[response.length - 1].user_id;
          setEmployeeID(lastUserID + 1);
      } else {
  ...
  const onButtonSubmit = async(e) => {
    ...
    try{
        if(!employeeID || !firstname || !lastname || !dpt){
            setCurrent(0);
            message.error('กรุณากรอกข้อมูลพนักงานให้ครบถ้วน');

        }else {
            setloading(true);
            await axios.post("http://192.168.10.114:88/bcc/user/add", userData);
            setEmployeeID('');
            setFirstname('');
            setLastname('');
            setDpt('');
        ...
        if (error.response && error.response.data === 'EmployeeID Ready') {
          message.error('รหัสพนักงานนี้มีอยู่แล้ว');
      } else {
          message.error('Processing fail');
      }
      console.log('post new user: ', error);
