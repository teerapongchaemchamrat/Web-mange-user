...
import Adduser from '../TKG/user_add';
import Edituser from '../TKG/user_edit';
...
export default function ManageUser () {
  ...
  const getData = async () => {
    try{
        setloading(true);
        const getuser = await axios.get('http://192.168.10.114:88/tkg/user/active');
  ...
  const showModalRemove = async (record) => {
    ...
    try{
      setloading(true);
      await axios.put(`http://192.168.10.114:88/tkg/user/remove/${record.user_id}`, Data);
      ...
      <Document>
        <Page size="A4" style={styles.page}>
          ...
          <View style={styles.section}>
            {/* for user */}
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '15.7%', marginLeft: '20%'}}>
              <Text>{record.first_name} {record.last_name}</Text>
            </div>
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '15.7%', marginLeft: '67%'}}>
              <Text>{record.dpt} </Text>
            </div>
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '23.5%', marginLeft: '20%'}}>
              <Text>{record.com_id} </Text>
            </div>
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '27.5%', marginLeft: '20%'}}>
              <Text>{record.com_pass} </Text>
            </div>
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '35.7%', marginLeft: '20%'}}>
              <Text>{record.email_id} </Text>
            </div>
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '39.7%', marginLeft: '20%'}}>
              <Text>{record.email_pass} </Text>
            </div>
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '49.7%', marginLeft: '20%'}}>
              <Text>{record.dcc_id} </Text>
            </div>
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '53.7%', marginLeft: '20%'}}>
              <Text>{record.dcc_pass} </Text>
            </div>
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '49.7%', marginLeft: '68%'}}>
              <Text>{record.infor_id} </Text>
            </div>
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '53.7%', marginLeft: '68%'}}>
              <Text>{record.infor_pass} </Text>
            </div>
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '62%', marginLeft: '20%'}}>
              <Text>{record.print_id} / {record.print_pass} </Text>
            </div>

            {/* for staff */}
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '86.8%', marginLeft: '20%'}}>
              <Text>{record.first_name} {record.last_name}</Text>
            </div>
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '86.8%', marginLeft: '67%'}}>
              <Text>{record.dpt} </Text>
            </div>
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '94.8%', marginLeft: '20%'}}>
              <Text>{record.com_id} </Text>
            </div>
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '98.8%', marginLeft: '20%'}}>
              <Text>{record.com_pass} </Text>
            </div>
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '106.9%', marginLeft: '20%'}}>
              <Text>{record.email_id} </Text>
            </div>
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '110.9%', marginLeft: '20%'}}>
              <Text>{record.email_pass} </Text>
            </div>
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '121%', marginLeft: '20%'}}>
              <Text>{record.dcc_id} </Text>
            </div>
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '125%', marginLeft: '20%'}}>
              <Text>{record.dcc_pass} </Text>
            </div>
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '121%', marginLeft: '68%'}}>
              <Text>{record.infor_id} </Text>
            </div>
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '125%', marginLeft: '68%'}}>
              <Text>{record.infor_pass} </Text>
            </div>
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '133%', marginLeft: '13%'}}>
              <Text>{record.print_id} </Text>
            </div>
            <div style={{position: 'absolute',fontSize: '12px', marginTop: '133%', marginLeft: '37%'}}>
              <Text>{record.print_pass} </Text>
            </div>
          </View>
          <Image src="/image/TKG_userform.jpg" style={styles.backgroundImage} />
        </Page>
      </Document>
    );
